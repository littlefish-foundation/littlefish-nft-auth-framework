...
describe('WalletProvider Component', () => {
    const TestComponent = () => {
        const { connectWallet, disconnectWallet, isConnected, wallets, connectedWalletId, assets, decodeHexToAscii } = useWallet();
                ...
                {!isConnected ? (
                    ...
                    <div>
                        ...
                        {walletAssets && walletAssets.map((item, index) => ( // Check for walletAssets being non-null
                            <pre key={index}>
                                PolicyID: {item.policyID}, Name: {item.assetName}, Amount: {item.amount}
                            </pre>
                        ))}
                    </div>
                )}
    ...
    test('connect wallet and handle disconnect', async () => {
        ...
        const disconnectButton = queryByText('Disconnect Wallet');
        if (disconnectButton) {
